    ...
    stages {
        stage('Checkout Github') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: "${GIT_CREDENTIALS_ID}", url: 'https://github.com/andela-Taiwo/Study-Buddy.git']])
            }
        }        
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: andelataiwo/studybuddy:.*|image: andelataiwo/studybuddy:${IMAGE_TAG}|' manifests/deployment.yaml
                    """
                }
            }
        }
